...
// are exposed separately for self-service flows driven from tokenHub's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("tokenHub-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "tokenHub-store"
	defaultWaffoPancakeProductName = "tokenHub-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because tokenHub has no renewal-
// event handling; Pancake auto-renewing without tokenHub extending user
